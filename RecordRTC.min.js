...
// Last time updated: 2016-08-28 3:42:16 AM UTC
...
function RecordRTC(mediaStream, config) {
    ...
    var mediaRecorder, self = this,
        WARNING = 'It seems that "startRecording" is not invoked for ' + config.type + " recorder.",
        returnObject = {
...
function MediaStreamRecorder(mediaStream, config) {
    ...
    if (config = config || {
        ...
        config.mimeType && -1 !== config.mimeType.toString().toLowerCase().indexOf("audio") || (config.mimeType = isChrome ? "audio/webm" : "audio/ogg"), config.mimeType && "audio/ogg" !== config.mimeType.toString().toLowerCase() && navigator.mozGetUserMedia && (config.mimeType = "audio/ogg")
    }
    this.record = function() {
        self.blob = null;
        var recorderHints = config;
        config.disableLogs || console.log("Passing following config over MediaRecorder API.", recorderHints), mediaRecorder && (mediaRecorder = null), isChrome && !isMediaRecorderCompatible() && (recorderHints = "video/vp8");
        try {
            mediaRecorder = new MediaRecorder(mediaStream, recorderHints)
        } catch (e) {
            mediaRecorder = new MediaRecorder(mediaStream)
        }
        "canRecordMimeType" in mediaRecorder && mediaRecorder.canRecordMimeType(config.mimeType) === !1 && (config.disableLogs || console.warn("MediaRecorder API seems unable to record mimeType:", config.mimeType)), mediaRecorder.ignoreMutedMedia = config.ignoreMutedMedia || !1, mediaRecorder.ondataavailable = function(e) {
            self.dontFireOnDataAvailableEvent || !e.data || !e.data.size || e.data.size < 100 || self.blob || (self.blob = config.getNativeBlob ? e.data : new Blob([e.data], {
        ...
        mediaRecorder && (this.recordingCallback = function(blob) {
            mediaRecorder = null, callback && callback(blob)
...
function StereoAudioRecorder(mediaStream, config) {
    ...
    config.leftChannel === !0 && (numberOfAudioChannels = 1), 1 === config.numberOfAudioChannels && (numberOfAudioChannels = 1), config.disableLogs || console.debug("StereoAudioRecorder is set to record number of channels: ", numberOfAudioChannels), this.record = function() {
        ...
        recording = !1, mergeLeftRightBuffers({
            ...
            self.blob = new Blob([view], {
                type: "audio/wav"
            }), self.buffer = new ArrayBuffer(view.buffer.byteLength), self.view = view, self.sampleRate = sampleRate, self.bufferSize = bufferSize, self.length = recordingLength, callback && callback(), isAudioProcessStarted = !1
        })
    ...
    if (-1 === legalBufferValues.indexOf(bufferSize) && (config.disableLogs || console.warn("Legal values for buffer-size are " + JSON.stringify(legalBufferValues, null, "	"))), context.createJavaScriptNode) jsAudioNode = context.createJavaScriptNode(bufferSize, numberOfAudioChannels, numberOfAudioChannels);
    else {
...
function CanvasRecorder(htmlElement, config) {
    ...
    _isChrome && matchArray && matchArray[2] && (chromeVersion = parseInt(matchArray[2], 10)), _isChrome && 52 > chromeVersion && (isCanvasSupportsStreamCapturing = !1);
    var globalCanvas, mediaStreamRecorder;
    if (isCanvasSupportsStreamCapturing)
        if (config.disableLogs || console.debug("Your browser supports both MediRecorder API and canvas.captureStream!"), htmlElement instanceof HTMLCanvasElement) globalCanvas = htmlElement;
        else {
    ...
    this.record = function() {
        ...
        whammy.frames.forEach(function(frame, idx) {
            ...
            config.disableLogs || console.debug(framesRemaining + "/" + framesLength + " frames remaining"), config.onEncodingCallback && config.onEncodingCallback(framesRemaining, framesLength);
            var webp = frame.image.toDataURL("image/webp", 1);
            whammy.frames[idx].image = webp
        }), config.disableLogs || console.debug("Generating WebM"), callback()
    }, this.stop = function(callback) {
        isRecording = !1;
        var that = this;
        return isCanvasSupportsStreamCapturing && mediaStreamRecorder ? void mediaStreamRecorder.stop(callback) : void this.getWebPImages(function() {
            whammy.compile(function(blob) {
                config.disableLogs || console.debug("Recording finished!"), that.blob = blob, that.blob.forEach && (that.blob = new Blob([], {
                    type: "video/webm"
...
function WhammyRecorder(mediaStream, config) {
    ...
    function dropBlackFrames(_frames, _framesToCheck, _pixTolerance, _frameTolerance, callback) {
        ...
        var context2d = localCanvas.getContext("2d"),
            resultFrames = [],
            checkUntilNotBlack = -1 === _framesToCheck,
    ...
    config = config || {}, config.frameInterval || (config.frameInterval = 10), config.disableLogs || console.log("Using frames-interval:", config.frameInterval), this.record = function() {
        config.width || (config.width = 320), config.height || (config.height = 240), config.video || (config.video = {
            ...
            config.initCallback && config.initCallback()
        }, video.width = config.video.width, video.height = config.video.height), video.muted = !0, video.play(), lastTime = (new Date).getTime(), whammy = new Whammy.Video, config.disableLogs || (console.log("canvas resolutions", canvas.width, "*", canvas.height),
            console.log("video width/height", video.width || canvas.width, "*", video.height || canvas.height)), drawFrames(config.frameInterval)
    };
    var isStopDrawing = !1;
    this.stop = function(callback) {
        isStopDrawing = !0;
...
function GifRecorder(mediaStream, config) {
    ...
    this.record = function() {
        ...
        lastAnimationFrame && cancelAnimationFrame(lastAnimationFrame), endTime = Date.now(), this.blob = new Blob([new Uint8Array(gifEncoder.stream().bin)], {
            type: "image/gif"
...
var isEdge = !(-1 === navigator.userAgent.indexOf("Edge") || !navigator.msSaveBlob && !navigator.msSaveOrOpenBlob),
    ...
    isChrome = !isOpera && !isEdge && !!navigator.webkitGetUserMedia,
